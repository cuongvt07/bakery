                ...
                foreach ($this->distributionData as $batchId => $products) {
                    foreach ($products as $productId => $quantity) {
                        ...
                        PhanBoHangDiemBan::create([
                            ...
                            'ghi_chu' => "Phân bổ từ xưởng (Mẻ: " . ($batch->ma_me ?? 'N/A') . ")",
                        ]);
                        ...
                        \App\Models\LichSuCapNhatMe::create([
                            ...
                            'san_pham_id' => $productId,
                            'diem_ban_id' => $this->selectedAgencyId,
                            'loai' => \App\Models\LichSuCapNhatMe::LOAI_PHAN_BO,
                            'nguoi_cap_nhat_id' => Auth::id(),
                            'so_luong_doi' => $quantity,
                            'du_lieu_cu' => 0,
                            'du_lieu_moi' => $quantity,
                            'ghi_chu' => "Xuất xưởng phân bổ sang " . ($this->agencies->find($this->selectedAgencyId)->ten_diem_ban ?? 'Điểm bán')
                        ]);
