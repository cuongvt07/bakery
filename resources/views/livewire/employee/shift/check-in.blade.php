<div class="min-h-screen bg-gray-50 pb-20">
        ...
        @if(!$hasActiveShift)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 text-center">
                ...
                @else
                    ...
                    <a href="{{ route('employee.shifts.register') }}" wire:navigate class="mt-3 inline-block px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg text-sm font-medium hover:bg-indigo-100 transition-colors">
                        Đăng ký ca
                    </a>
                @endif
            </div>
                ...
                @foreach($todayShifts as $s)
                    ...
                    <button wire:click="selectShift({{ $s->id }})" 
                        ...
                        <div class="flex justify-between items-center mb-1">
                            ...
                            <span class="text-xs font-bold px-2 py-0.5 rounded {{ $isCompleted ? 'bg-gray-200 text-gray-500' : 'bg-blue-100 text-blue-700' }}">
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div class="{{ $checkoutWarningType === 'late' ? 'bg-red-600' : ($checkoutWarningType === 'early' ? 'bg-yellow-600' : 'bg-green-600') }} px-6 py-4">
                <h3 class="text-white font-bold text-lg flex items-center">
                    <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
