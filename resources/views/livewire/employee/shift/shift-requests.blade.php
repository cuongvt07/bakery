<div class="p-4 space-y-4">
    ...
    <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        <button wire:click="setFilter('')"
            class="px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap {{ $filterStatus === '' ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-200 text-gray-700' }}">
            Tất cả
        </button>
        <button wire:click="setFilter('cho_duyet')"
            class="px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap {{ $filterStatus === 'cho_duyet' ? 'bg-amber-500 text-white' : 'bg-white border border-gray-200 text-gray-700' }}">
            Chờ duyệt
        </button>
        <button wire:click="setFilter('da_duyet')"
            class="px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap {{ $filterStatus === 'da_duyet' ? 'bg-green-600 text-white' : 'bg-white border border-gray-200 text-gray-700' }}">
            Đã duyệt
        </button>
        <button wire:click="setFilter('tu_choi')"
            class="px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap {{ $filterStatus === 'tu_choi' ? 'bg-red-600 text-white' : 'bg-white border border-gray-200 text-gray-700' }}">
            Từ chối
        </button>
    </div>

    {{-- Create New Request Button --}}
    <button wire:click="openRequestModal('xin_ca')"
        class="w-full btn-mobile bg-indigo-600 text-white shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">
        ➕ Tạo yêu cầu mới
    </button>
        ...
        @forelse($requests as $request)
            <div class="bg-white rounded-xl shadow-sm p-4">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        <span
                            class="px-3 py-1 rounded-full text-xs font-medium {{ match ($request->loai_yeu_cau) {
                    ...
                    <span
                        class="px-3 py-1 rounded-full text-xs font-medium {{ match ($request->trang_thai) {
                ...
                <div class="text-sm text-gray-700 mb-3">
                    <strong>Lý do:</strong> {{ $request->reason_text }}
                </div>
                ...
                <div class="flex items-center justify-between mt-3 pt-3 border-t">
                    ...
                    @if ($request->trang_thai === 'cho_duyet')
                        <button wire:click="cancelRequest({{ $request->id }})" wire:confirm="Hủy yêu cầu này?"
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <select wire:model="requestType" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                            ...
                            <option value="ticket">🚨 Ticket khẩn cấp</option>
                        </select>
                    </div>
                    ...
                    @if ($requestType === 'xin_ca')
                        <div>
                            ...
                            <input type="date" wire:model="requestDate"
