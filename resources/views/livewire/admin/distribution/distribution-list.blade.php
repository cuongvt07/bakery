    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
        <div class="grid grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select wire:model.live="selectedAgency"
                    class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-lg">
                    ...
                    @foreach ($agenciesData as $agencyId => $data)
                        <tr class="hover:bg-gray-50">
                            <td class="px-3 py-2 text-sm font-semibold text-gray-900">
                                {{ $data['agency']->ten_diem_ban }}</td>
                            <td class="px-3 py-2 text-sm font-bold text-blue-600">
                                {{ number_format($data['total_quantity']) }}</td>
                            <td class="px-3 py-2 text-sm text-gray-600">{{ $data['total_items'] }}</td>
                            <td class="px-3 py-2 text-sm">
                                <span
                                    class="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-semibold">{{ $data['status_counts']['da_nhan'] }}</span>
                            </td>
                            <td class="px-3 py-2 text-sm">
                                <span
                                    class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs font-semibold">{{ $data['status_counts']['chua_nhan'] }}</span>
                            </td>
                            <td class="px-3 py-2 text-center">
                                <button wire:click="toggleDetails('{{ $date }}', {{ $agencyId }})"
                                    class="px-3 py-1 text-xs bg-indigo-100 text-indigo-700 rounded hover:bg-indigo-200 flex items-center gap-1 mx-auto">
                                    @if ($expandedRows[$date . '_' . $agencyId] ?? false)
                                        ▼ Thu gọn
                                    @else
                                        ▶ Chi tiết
                                    @endif
                        ...
                        @if ($expandedRows[$date . '_' . $agencyId] ?? false)
                            @php
                                $details = \App\Models\PhanBoHangDiemBan::with([
                                    'product',
                                    'productionBatch',
                                    'nguoiNhan',
                                ])
                            ...
                            @endphp
                            <tr class="bg-gray-50">
                                <td colspan="6" class="px-0 py-0 overflow-hidden">
                                    <div class="animate-[slideDown_0.3s_ease-out]">
                                        <div class="px-3 py-3">
                                            <div
                                                ...
                                                <table class="min-w-full text-sm">
                                                    <thead class="bg-gray-100">
                                                        <tr>
                                                            <th
                                                                class="px-3 py-2 text-left text-xs font-medium text-gray-600">
                                                                Mẻ
                                                                SX</th>
                                                            <th
                                                                class="px-3 py-2 text-left text-xs font-medium text-gray-600">
                                                            ...
                                                            <th
                                                                ...
                                                                Trạng thái</th>
                                                            <th
                                                                class="px-3 py-2 text-center text-xs font-medium text-gray-600">
                                                                Thao tác</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="divide-y divide-gray-200">
                                                        @foreach ($details as $item)
                                                            <tr class="hover:bg-gray-50">
                                                                <td class="px-3 py-2 font-semibold text-gray-900">
                                                                    #{{ $item->productionBatch->ma_me ?? 'N/A' }}</td>
                                                                <td class="px-3 py-2">
                                                                    <span
                                                                        class="px-2 py-0.5 rounded text-xs {{ $item->buoi === 'sang' ? 'bg-yellow-100 text-yellow-800' : 'bg-orange-100 text-orange-800' }}">
                                                                        {{ ucfirst($item->buoi) }}
                                                                    </span>
                                                                </td>
                                                                <td class="px-3 py-2 text-gray-900">
                                                                    {{ $item->product->ten_san_pham }}</td>
                                                                <td class="px-3 py-2 font-bold text-blue-600">
                                                                    {{ number_format($item->so_luong) }}</td>
                                                                <td class="px-3 py-2">
                                                                ...
                                                                <td class="px-3 py-2 text-center">
                                                                    @if ($item->trang_thai !== 'da_nhan')
                                                                        <button
                                                                            wire:click="deleteDistribution({{ $item->id }})"
                                                                            wire:confirm="Xóa phân bổ này?"
                                                ...
                                                @if ($details->isEmpty())
                                                    <div class="text-center text-gray-500 py-4 text-sm">Không có dữ liệu
                                                    </div>
                                                @endif
                                            </div>
                                </td>
